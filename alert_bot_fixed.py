...
# -----------------------------------------------------------------------------
# 4) Notification Functions - FIXED ASYNC HANDLING
# -----------------------------------------------------------------------------
def send_telegram_message_sync(text):
    """Send message via Telegram bot - synchronous wrapper"""
...
def send_notification(text):
    """Send notification via available channels - FIXED"""
    # Try Telegram first
    telegram_sent = False
    if telegram_configured:
        telegram_sent = send_telegram_message_sync(text)
...
# -----------------------------------------------------------------------------
# 6) Main alert checking function - FIXED (NO ASYNC)
# -----------------------------------------------------------------------------
def check_prices_and_alert():
    """Main function to check prices and send alerts - FIXED"""
    ...
    for ticker, thresholds in config.WATCHLIST.items():
        ...
        if upper is not None and latest_price >= upper and not alerted_status[ticker]["above"]:
            ...
            send_notification(text)  # FIXED - no await needed
...
if __name__ == "__main__":
    ...
    # Schedule the price checking - FIXED (no async wrapper needed)
